...
const owner = ["1173547185758015498"]
  ...
  if (message.content.match(mention)) {
    const lawde = new MessageActionRow()
      ...
      const embed = new MessageEmbed()
        ...
        .setFooter(`Made with 💖 By TEC`, message.guild.iconURL({dynamic: true}))
 .setThumbnail("https://cdn.discordapp.com/emojis/1199357089395527723.webp?size=128&quality=lossless")
  ...
  np = [];
      ...
      if(command.voteonly){
        ...
        if(!vote){
          ...
          .setDescription(`${client.emoji.wrong} | **This Is A Vote Required Command So Vote Us Now By [Clicking Here](https://top.gg/bot/${client.user.id}/vote)**`)
          .setColor(client.embedColor)
   ...
   if (!channel.permissionsFor(message.guild.me)?.has(Permissions.FLAGS.EMBED_LINKS) && client.user.id !== userId) {
    ...
    if (command.owner) {
     if (!owner.includes(message.author.id)) {
        ...
        embed.setDescription(`${client.emoji.wrong} | Only My Developers Can Use This Command`);
        return message.channel.send({embeds: [embed]});
    }
    }
